...
class uberUsers
{
	...
	{
		dbquery("INSERT INTO users (username,password,mail,auth_ticket,rank,look,gender,motto,credits,activity_points,last_online,account_created) VALUES ('" . $username . "','" . $passwordHash . "','" . $email . "','','" . $rank . "','" . $figure . "','" . $sex . "','','1500','1500','','" . date('d-M-Y') . "')");		
	...
	{
		return mysql_num_rows(dbquery("SELECT null FROM users WHERE mail = '" . $email . "' AND password = '" . $password. "' LIMIT 1"));
	}
	function ValidateLogin($user_mail, $password)
	{
		if ($this->ValidateUser($user_mail, $password))
			return array(1, 0);
		else if ($this->ValidateUserByEmail($user_mail, $password))
			return array(1, 1);
		else
			return array(0, null);
	}
	...
	function GetFriendRequests($id){
      $amount = mysql_num_rows(dbquery("SELECT * FROM messenger_requests WHERE `to_id` = '" . $id . "'"));
	...
	function CheckSSO($id)
	{
		...
		function AddOrUpdateClub($id, $subID, $sec) // HC/VIP Subscription Function
	{
	    $sql = dbquery("SELECT * FROM user_subscriptions WHERE user_id = '" . $id . "' LIMIT 1"); // Select Subscription
		...
		if(mysql_num_rows($sql) > 0)
		{
		   ...
		   dbquery("UPDATE user_subscriptions SET timestamp_expire =  '" . $secEnd . "', subscription_id = '" . $subID . "' WHERE user_id = '" . $id . "'  LIMIT 1");  // Renew Subscription -> Not Funcional but here's the example code.
		   
		   //dbquery("UPDATE users SET rank = '2' WHERE rank = '1' AND  id = '" . $id . "' LIMIT 1"); -> Maybe for buy VIP on uberCms?
		   
		}
		else
		{	
		   dbquery("INSERT INTO user_subscriptions (user_id,timestamp_expire,timestamp_activated,subscription_id) VALUES ('" . $id . "','" . $secEnd . "','" . $secInit . "','" . $subID . "')"); // Insert Subscription buy into his table with her respective User and subscription data.
	    }
	}
	...
	{
		$sql = dbquery("SELECT timestamp_activated, timestamp_expire FROM user_subscriptions WHERE user_id = '" . $id . "' LIMIT 1");
	...
	function hasVIP($id)
	{
		$sql = dbquery("SELECT timestamp_activated, timestamp_expire FROM user_subscriptions WHERE subscription_id = 'habbo_vip' AND user_id = '" . $id . "' LIMIT 1");
