<?php
$page['id']="1";
$page['sub_id']="1";
$page['name']="pixels";
$page['redirect']="1";
require_once("./includes/core.php");
?>
<div id="overlay"></div>
<div id="header-container">
	<div id="header" class="clearfix">
		<h1><a href="<?php echo get_settings("hotel_url"); ?>/"></a></h1>
<div id="subnavi">     <div id="subnavi-search">
         <div id="subnavi-search-upper">

               <ul id="subnavi-search-links">

                               

                       <li>

                               <form action="logout.php" method="post">

                                       <button type="submit" id="signout" class="link"><span>Logout</span></button>

                               </form>

                       </li>

               </ul>         </div>
     </div>
     <div id="to-hotel">
                 <a href="client" class="new-button green-button" target="104b4c2297835b0959575b9926d7729ebc753c0e" onclick="HabboClient.openOrFocus(this); return false;"><b>Enter in <?php echo get_title(); ?></b><i></i></a>
     </div>
 </div>
 <script type="text/javascript">

 L10N.put("purchase.group.title", "Create a Group");
 document.observe("dom:loaded", function() {
     $("signout").observe("click", function() {
         HabboClient.close();
     });
 });
 </script><ul id="navi">        <li>
                   <strong>
                           <?php echo get_userinfo("username"); ?> ( <img src="http://cotendo.habbo.com/habboweb/63_1dc60c6d6ea6e089c6893ab4e0541ee0/<?php echo $habboweb ?>/web-gallery/v2/images/rpx/icon_habbo_small.png" style="margin-bottom:-2px;" /> )</i>
                   </strong>
                 <span></span>
               </li>
<li>

			<a href="/news">Community</a>
			<span></span>
		</li>
		<li  class="metab selected">
<strong>
			Credits</strong>
			<span></span>
		</li>
		<li>

			<a href="/safety">Safety</a>
			<span></span>
		</li></ul>

        <div id="habbos-online"><div class="rounded"><span><?php echo get_users_online(); ?> <?php echo $language['members_online']; ?></span></div></div>

	</div></div><div id="content-container">

<div id="navi2-container" class="pngbg">
    <div id="navi2" class="pngbg clearfix">
	<ul>
			<li>
				<a href="/credits">Credits</a>
				
			</li>
			<li  class="selected">
				Pixels
				
			</li>
	</ul>
    </div>
</div>
<div id="container">
	<div id="content" style="position: relative" class="clearfix">
    <div id="column1" class="column">

			     		
				<div class="habblet-container ">		
						<div class="cbb clearfix pixelblue ">
	
							<h2 class="title">Learn how to get pixel and profit!
							</h2>
						<div class="pixels-infobox-container">
    <div class="pixels-infobox-text">
            <h3>You can get Pixel in several ways:</h3>
            <ul>
                <li><p>1. Entering Hotel</p></li>

                <li><p>2. Moving on your time in the hotel - more 'time we walk, the more' pixels you get!</p></li>
                <li><p>3. Reaching Goals, working as a guide and giving respect to others.</p></li>
                <li><p>4. becoming HC</p></li>
            </ul>
            <p>How to spend it? Check out the new section of the catalog 'Pixel Shop'!</p>
            <p><a href="/FAQ">FAQ</a></p>

    </div>
</div>
	
						
					</div>
				</div>
				<script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>
			 

</div>
<div id="column2" class="column">
			     		
				<div class="habblet-container ">		
						<div class="cbb clearfix pixelgreen ">
	
							<h2 class="title">Rent NOW!
							</h2>
						<div id="pixels-info" class="box-content pixels-info">
    <div class="pixels-info-text clearfix">
        <img class="pixels-image" src="./web-gallery/v2/images/pixelpage_effectmachine.png" alt=""/>
        <p class="pixels-text">Create a room to cry using these special effects!
Your friends will be amazed!</p>
    </div>
</div>
	
						
					</div>
				</div>
				<script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>
			 

			     		
				<div class="habblet-container ">		
						<div class="cbb clearfix pixellightblue ">
	
							<h2 class="title">Effects?
							</h2>
						<div id="pixels-info" class="box-content pixels-info">
    <div class="pixels-info-text clearfix">
        <img class="pixels-image" src="./web-gallery/v2/images/pixelpage_personaleffect.png" alt=""/>
        <p class="pixels-text"> It 's your chance to do what you always dreamed of. Want to travel on a flying carpet or do you prefer a parade on the red carpet? Whatever you want you can finally 'have it!</p>
    </div>
</div>
	
						
					</div>
				</div>
				<script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>
			 

			     		
				<div class="habblet-container ">		
						<div class="cbb clearfix pixeldarkblue ">
	
							<h2 class="title">Special Offers?
							</h2>
						<div id="pixels-info" class="box-content pixels-info">
    <div class="pixels-info-text clearfix">
        <img class="pixels-image" src="./web-gallery/v2/images/pixelpage_discounts.png" alt=""/>
        <p class="pixels-text">Earning Pixels get discounts on a wide variety 'of Furni - see how much you can save in our section dedicated to!</p>
    </div>
</div>
	
						
					</div>
				</div>
				<script type="text/javascript">if (!$(document.body).hasClassName('process-template')) { Rounder.init(); }</script>
			 

</div>
<script type="text/javascript">
HabboView.run();
</script>
<div id="column3" class="column">
</div>
<!--[if lt IE 7]>
<script type="text/javascript">
Pngfix.doPngImageFix();
</script>
<!--[if lt IE 8
]>
<script type="text/javascript">
Pngfix.doPngImageFix();
</script>
<![endif]-->
<div id="footer">
    
    <p class="copyright"><a href="/iot/go.php" target="_self"><font color="#0000ff">Contattaci</font></a><font color="#0000ff"> <font color="#0000ff"> | <a href="/FAQ" target="_self"><font color="#0000ff">FAQ</font></a><font color="#0000ff"> <font color="#0000ff"> | <a href="/disclaimer.php" target="_self"><font color="#0000ff">Termini di Servizio</font></a><font color="#0000ff"> <font color="#0000ff"> | <a href="/group_profile.php?id=1381" target="_self"><font color="#0000ff">Consigli di Sicurezza</font></a></font></font></font></p><font color="#0000ff"><font color="#0000ff"><font color="#0000ff"><font color="#0000ff">
    
</div></div>

</div>


<script src="http://www.google-analytics.com/ga.js" type="text/javascript"></script>
<script type="text/javascript">
var pageTracker = _gat._getTracker("UA-448325-20");
pageTracker._initData();
pageTracker._trackPageview();
</script>
<script type="text/javascript">
if (window.location.href.indexOf("/register/welcome") != -1) {
        <!-- Google Code for Registration Conversion Page HL-14188 BEGIN -->
        var google_conversion_id = 1042363710;
        var google_conversion_language = "it";
        var google_conversion_format = "2";
        var google_conversion_color = "ffffff";
        var google_conversion_label = "u3yICManSBC-6oTxAw";

        document.write('<' + '<script language="JavaScript" type="text/javascript" src="http://www.googleadservices.com/pagead/conversion.js">' +        '>' + '</' + 'script' + '>');
        <!-- Google Code for signup Conversion Page HL-14188 END -->

}
</script>

<!-- Start Quantcast tag -->
<script type="text/javascript" src="http://edge.quantserve.com/quant.js"></script>
<script type="text/javascript">_qacct="p-b5UDx6EsiRfMI";quantserve();</script>
<noscript>
<a href="http://www.quantcast.com/p-b5UDx6EsiRfMI" target="_blank"><img src="http://pixel.quantserve.com/pixel/p-b5UDx6EsiRfMI.gif" style="display: none" border="0" height="1" width="1" alt="Quantcast"/></a>
</noscript>
<!-- End Quantcast tag -->

<script type="text/javascript">
//<![CDATA[
google_color_border = "ffff33";
google_color_bg = "ffffff";
google_color_link = "ffff33";
google_color_url = "ffff00";
google_color_text = "000000";
document.write('<s'+'cript type="text/javascript" src="http://ad.altervista.org/js.ad/size=468X60/r='+new Date().getTime()+'"><\/s'+'cript>');
//]]>
</script>

</body>
</html>