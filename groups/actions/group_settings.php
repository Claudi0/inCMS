    ...
    <div id="group-settings-data" class="group-settings-pane">
      ...
      <div id="group-identity-area">
        <div id="group-name-area">
          ...
          <input type="text" name="group_name" id="group_name" onKeyUp="GroupUtils.validateGroupElements('group_name', 30, 'Has alcanzado el límite');" value="<?php echo CleanText($row['name']); ?>"/><br />
        </div>
        ...
        <div id="group-description-area">
          ...
          <textarea name="group_description" id="group_description" onKeyUp="GroupUtils.validateGroupElements('group_description', 255, 'La descripción del Grupo es demasiado larga');"><?php echo CleanText($row['description']); ?></textarea>
        </div>
      </div>
...
<div id="room-settings-id" class="group-settings-pane-wide group-settings-selection">
	...
	<?php while($row = mysql_fetch_assoc($sql)) {
    	...
    	<li class="<?php if($System->isEven($i) == "even") { echo "even"; } else { echo "odd"; } ?>">
    		<input type="radio" name="roomId" value="<?php echo $row['id']; ?>" <?php if($group['roomid'] == $row['id']) { echo "checked=\"checked\""; } ?> /><a href="/client?forwardId=2&roomId=<?php echo $row['id']; ?>" onclick="HabboClient.roomForward(this, '<?php echo $row['id']; ?>', 'private'); return false;" target="<?php echo $myrow['client_token']; ?>" class="room-enter">Entrada</a><div>
