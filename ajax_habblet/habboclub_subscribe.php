...
$vip_ini = time();
$check  = mysql_query("SELECT user_id, timestamp_expire FROM user_subscriptions WHERE user_id=".$id." AND subscription_id='habbo_club' ORDER BY timestamp_expire DESC LIMIT 1");
...
mysql_query("DELETE FROM user_subscriptions WHERE user_id=".$id." AND subscription_id='habbo_club'");
mysql_query("INSERT INTO user_subscriptions(user_id, subscription_id, timestamp_activated, timestamp_expire) VALUES ('".$id."', 'habbo_club', '".$vip_ini."', '".$vip_end."')");
mysql_query("UPDATE users SET credits='".$final."' WHERE username = '" .USER_NAME. "'");
...
$vip_end = time() + ($days*24*3600);
$vip_ini = time();
$check  = mysql_query("SELECT user_id, timestamp_expire FROM user_subscriptions WHERE user_id=".$id." AND subscription_id='habbo_vip' ORDER BY timestamp_expire DESC LIMIT 1");
$total_records = mysql_num_rows($check);
if ($total_records>0){
$row = mysql_fetch_assoc($check);
$current_exp = $row['timestamp_expire'];
if($current_exp > time()){
$vip_end = $vip_end + ($current_exp - time());}};
mysql_query("DELETE FROM user_subscriptions WHERE user_id=".$id." AND subscription_id='habbo_vip'");
mysql_query("INSERT INTO user_subscriptions(user_id, subscription_id, timestamp_activated, timestamp_expire) VALUES ('".$id."', 'habbo_vip', '".$vip_ini."', '".$vip_end."')");
mysql_query("UPDATE users SET coins='".$final."' WHERE username = '" .USER_NAME. "'");
...
<p>Error al realizar la compra. Por favor, int&#233;ntalo de nuevo.</p>
 ...
 <p><a href="credits" onclick="window.location = \'/shop/Tienda\'" class="new-button"><b>Obtener Coins</b><i></i></a></p>
<div class="clear"></div>';};
}else{echo 'Error procesando tu solicitud.';};};
}else{echo 'Error procesando tu solicitud.';};
?>